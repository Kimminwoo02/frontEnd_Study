...
const App = () => { 
  ...
  const [ searchItimport { useState } from 'react';
import React from 'react';
...
const AddItem = ({ newItem, setNewItem, handleSubmit })=>{
    ...
    return(
        <form  className="addForm" onSubmit ={ handleSubmit }>
            <label htmlFor="addItem"> 상품명 입력하세요</label>
            <input
                    autoFocus
                    ref={inputRef}
                    id='addItem'
                    ...
                    onChange={(e) => setNewItem(e.target.value)}
          />
            <button type="submit" onClick={  ()=> inputRef.current.focus() } > 등록  </button>
        </form>
    )
}
